  ...
  <div class="publishPost">
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
          ...
          <el-form-item>
            ...
            <el-button type="primary" @click="onSubmitOntiming('form')">定时发布</el-button>
...
import { addPost,getPostOptionsStatus,getTags,addPostOnTiming }  from '@/api/api'
  ...
  data() {
    ...
    methods: {
      ...
      onSubmitOntiming(form) {
          this.$refs[form].validate((valid) => {
              if(valid){
                  ...
                  addPostOnTiming(this.form).then(res =>{
                      if(res.code === 100000){
                          console.log('onSubmit',res)
                          this.$modal.msgSuccess("定时发布成功");
                          this.resetForm()
                      }else{
                          this.$modal.msgSuccess("定时发布失败");
                          //this.$message({message: res.data,type : 'error'})
                      }
      ...
      getAllStatus(){
          ...
          .then(res =>{
                ...
                if(res.code === 100000){
                    ...
                    this.form.status = res.data[0].statusName 
                    //loading = false
                }
          })
      },
      ...
      resetForm(){
        ...
        this.form.status = this.docStatus[0].statusName,
